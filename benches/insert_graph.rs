...
// ---------------------------------------------------------------------------
// TUI drawing (only when `tui` feature is enabled)
// ---------------------------------------------------------------------------
...
fn main() {
    ...
    // Timing macro: for a given size, create fresh maps (no pre-alloc) and insert `size` entries.
    // Matches criterion: normalizes work so total inserts per round ≈ max_size.
    macro_rules! time_inserts {
        ($size:expr, $impl_name:expr, $new_map:expr) => {{
            let size = $size;
            let maps_per_round = (max_size / size).max(1);
            let total_inserts_per_round = maps_per_round * size;
            let mut total_ns = 0u128;
            for _ in 0..ROUNDS {
                let start = Instant::now();
                for _ in 0..maps_per_round {
                    let mut map = $new_map;
                    for idx in 0..size {

...
const PREALLOCATED_INPUT_SIZE: usize = INPUT_SIZE * 2;
...
fn bench_insert_preallocated(c: &mut Criterion) {
    ...
    group.bench_function("pomap", |b| {
        let mut map: PoMap<u64, u64> = PoMap::with_capacity(PREALLOCATED_INPUT_SIZE);
        b.iter(|| {
            let key = keys[cursor];
            let val = values[cursor];
            black_box(map.insert(key, val));
    ...
    group.bench_function("std_hashmap", |b| {
        let mut map: HashMap<u64, u64> = HashMap::with_capacity(PREALLOCATED_INPUT_SIZE);
        b.iter(|| {
            let key = keys[cursor];
            let val = values[cursor];
            black_box(map.insert(key, val));
